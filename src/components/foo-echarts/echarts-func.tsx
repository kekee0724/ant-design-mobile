...
export const ECharts: FC<EChartsProps> = forwardRef((props, _ref) => {
  const defaultTheme = {
    color: [
      '#5b8ff9',
      '#cdddfd',
  ...
  const [state, setState] = useState<EChartsState>(initialState)
  let echartRef: any
  ...
  useEffect(() => {
    const instance = echartRef && echartRef.getEchartsInstance()
    if (instance) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('bi:theme', e => setTheme(e))
    return () => {
      window.removeEventListener('bi:theme', e => setTheme(e))
    }
  }, [])

  const refreshOption = () => {
    echartRef.getEchartsInstance().setOption(props)
  }
