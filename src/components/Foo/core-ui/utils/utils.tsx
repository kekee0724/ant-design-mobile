 ...
 */
export function transObjToString(obj: any, symbol = ',') {
  if (!obj) return
  let string = ''
 ...
 *          value：如果有重复，则返回该值
 */
export function isExistInObj(arr: any[], key: string | number) {
  let isRepeat = false
  let value
...
export function arrToTree(
  ...
  arr.forEach(item => {
    const parent = map[item[parentidMap]]
    if (parent) {
...
export function getExtraAttrs(props: any = {}) {
