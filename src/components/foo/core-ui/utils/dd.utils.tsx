...
import { attachService, browser, getObjectProp } from '../../core'
import { download } from './common'
import { ToastInfo } from './utils'

declare let window: Window & typeof globalThis & { [key: string]: any }
 ...
 * @param item 文件信息
 */
export function goToPreview(_this: any, item: any) {
  _this.goTo({
    pathname: 'preview',
    state: {
      url: item.filePath,
 ...
 * @param item 文件信息
 */
export function isOnlineDoc(_this: any, item: any) {
 ...
 * @param item 文件信息
 */
export function previewFile(_this: any, item: any) {
  if (browser.versions.weChatMini) {
    isOnlineDoc(_this, item)
  } else if (
    (ddkit && ddkit.env.platform !== 'notInDingTalk') ||
    browser.versions.weChat
  ) {
...
// 钉钉预览图片处理
export function previewFileC(
  url: string | URL | undefined,
  callback: () => void
) {
  if (
    browser.versions.weChatMini ||
    browser.versions.weChat ||
    (ddkit && ddkit.env.platform !== 'notInDingTalk')
  ) {
    window.open(url)
