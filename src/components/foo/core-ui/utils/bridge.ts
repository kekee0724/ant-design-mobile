...
function registerGlobalFunc(func: any) {
...
function registerTimeout<T>(
  ...
  const time =
      options.timeout >= 0
        ...
        : undefined,
    resolveID = registerGlobalFunc((d: T | PromiseLike<T> | undefined) => {
...
export function webkitPostMessage<T>(action: string, data?: T, options?: any) {
  return new Promise((resolve, reject) => {
    try {
      ...
      handler
        ? handler.postMessage(
            registerTimeout(resolve, reject, action, data, options)
          )
        : reject()
    } catch (error) {
      reject(error)
...
export function webViewJavascriptBridge<T>(
  action: string,
  data?: T,
  options?: any
) {
...
export function postBridgeMessage<T>(action: string, data?: T, options?: any) {
  return (!window['webkit'] ? webViewJavascriptBridge : webkitPostMessage)(
    action,
    data,
    options
  )
