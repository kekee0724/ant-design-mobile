...
export function checkStrategy(code: string, app = '') {
  ...
  return (strategy_cache[key] = app
    ? strategies.some((item: string) =>
        app.endsWith('_')
          ? item.startsWith(code + ':' + app)
          : item === code + ':' + app
      )
    : strategies.some(
        (item: string) =>
          item === code || item.substr(0, item.indexOf(':')) === code
      ))
}
...
export function checkStrategyPrefix(code: string) {
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] = strategies.some((item: string) =>
...
export function checkStrategyAny(...codes: string[]): boolean {
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] = codes.some(v => checkStrategy(v)))
}
...
export function checkStrategyAll(...codes: string[]): boolean {
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] = codes.every(v => checkStrategy(v)))
}
...
export function checkStrategyRegExp(exp: RegExp): boolean {
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] = strategies.some((v: string) => exp.test(v)))
}
...
export function checkStrategyRegExpAny(...exps: RegExp[]): boolean {
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] = exps.some(v => checkStrategyRegExp(v)))
}
...
export function checkStrategyRegExpAll(...exps: RegExp[]): boolean {
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] = exps.every(v => checkStrategyRegExp(v)))
}

export function checkStrategyUnit(
  code: string,
  unitId?: any,
  app?: string
): boolean {
  ...
  const key =
    'checkStrategyUnit:' + unitId + ':' + code + (app ? ':' + app : '')
  const filter = app
    ? (item: string) =>
        app.endsWith('_')
          ? item.startsWith(code + ':' + app)
  ...
  return strategy_cache.hasOwnProperty(key)
    ? strategy_cache[key]
    : (strategy_cache[key] =
        unitStrategies[unitId] && unitStrategies[unitId].some(filter))
}
