...
export function htmlInjectEncode(
  html: html,
  tagExp = /<\/?(iframe|script|link|style|frameset|frame)\b[^>]*?>/gi
) {
  return (
    html && html.replace(tagExp, d => `&lt;${d.substr(1, d.length - 2)}&gt;`)
  )
}

export function htmlInjectDecode(
  html: html,
  tagExp = /&lt;\/?(iframe|script|link|style|frameset|frame)\b.*?&gt;/gi
) {
