...
import { IDParam, RequestOptionsArgs } from '../http'
...
export class AttachService extends HttpService {
  ...
  getUploadChunkUrl(uploadId: string): string {
    return this.getRootUrl(
      this.transformUrl(this.resolveUrl('upload-chunk', uploadId))
    )
  }

  getPreviewUrl(id: IDParam, uploadId?: string, options?: RequestOptionsArgs) {
    return this.httpGet(
      this.resolveUrl('preview-url', id),
      this.resolveRequestParams({ uploadId }, options)
    )
  }

  downloadFile(uploadId: string, id: IDParam, options?: RequestOptionsArgs) {
    return this.httpGetDownload(
      this.resolveUrl('download', id),
      this.resolveRequestParams({ uploadId }, options)
    )
  }
}
