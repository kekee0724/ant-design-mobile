...
const disableTimeout = getObjectProp(client, 'http.disableTimeout', !1)

if (navigator && (navigator as any).connection) {
  const times: { [key: string]: number } = { '2g': 18000, '3g': 12000 }
        ...
        .then((res: Response) => {
          if (res.ok) resolve(res as any)
            ...
            res.json().then(d => {
              ...
              return (
                (((res as any).data = d),
                ((res as any).input = input),
                ((res as any).options = options)),
                emit(res),
                reject(d)
              )
            }, reject)
            ...
            ;(((res as any).errmsg = errmsg), ((res as any).input = input)),
              emit(res),
              reject(res)
          }
        })
