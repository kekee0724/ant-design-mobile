...
import { refreshTokens, verifyAuth } from './auth'
...
export class HttpClient {
  ...
  request<T = any>(
    ...
    return verifyAuth(options.allowAnonymous).then(
      verify =>
        verify
          ? this.handleRequest<T>(first, url, options)
  ...
  protected handleRequest<T = any>(
    ...
    return this.transformResponse(first, req, httpRequest(req)).then(
      null,
      (error: any) => this.handleResponseError(error, first, req)
    )
  }
  ...
  protected transformRequestInfo(
    first: string,
    url: string,
    options: RequestOptionsArgs = {}
  ) {
  ...
  protected transformRequest(
    ...
    return first instanceof HttpRequest
      ? (((first as any).urls = this.transformUrl(first.url)), first)
